...
async function askGroq(jid, userMsg) {
  ...
  const resp = await groq.chat.completions.create({
    ...
    messages: [
      { role: 'system', content: `Eres benny, un agente inmobiliario experto y amable. Ayudas a los clientes a encontrar propiedades usando las herramientas disponibles. Responde siempre en español, de forma concisa y profesional. Tu teléfono es . Tu WhatsApp directo: . Cuando el usuario pida propiedades usa la función de búsqueda disponible en el sistema.` },
      ...conversations[jid]
    ],
    max_tokens: 800
  });
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      try {
        ...
        const { buscarPorFiltros, todas } = require('./db');
        const isAdmin = ADMIN_JID && jid === ADMIN_JID;

        if (texto.toLowerCase().includes('ver todas') && isAdmin) {
          const props = todas();
          await sock.sendMessage(jid, { text: props.length ? props.map(p => `• ${p.nombre} - ${p.ubicacion} - $${p.precio.toLocaleString()}`).join('\n') : 'No hay propiedades.' });
          continue;
        }

        // Extract filters from message
        const ubicacionMatch = texto.match(/en ([\w\s]+?)(?:\s+de|\s+por|\s+con|\s*$)/i);
        const precioMaxMatch = texto.match(/hasta\s*\$?([\d,]+)/i);
        const precioMinMatch = texto.match(/desde\s*\$?([\d,]+)/i);
        const tipoMatch = texto.match(/\b(casa|departamento|depa|local|terreno|oficina)\b/i);

        const filtros = {
          ubicacion: ubicacionMatch?.[1]?.trim(),
          tipo: tipoMatch?.[1],
          precioMax: precioMaxMatch ? Number(precioMaxMatch[1].replace(/,/g,'')) : undefined,
          precioMin: precioMinMatch ? Number(precioMinMatch[1].replace(/,/g,'')) : undefined,
        };

        const propiedades = buscarPorFiltros(filtros);
        const contextoPropiedades = propiedades.length
          ? '\nPropiedades disponibles:\n' + propiedades.map(p =>
              `- ${p.nombre} | ${p.tipo} | ${p.ubicacion} | $${p.precio.toLocaleString()} | ${p.metraje}m²${p.link ? ' | '+p.link : ''}`
            ).join('\n')
          : '\nNo encontré propiedades con esos filtros.';

        const respuesta = await askGroq(jid, texto + contextoPropiedades);
